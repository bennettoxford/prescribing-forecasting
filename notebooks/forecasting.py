...
# === Data initiation functions === Creates initial values for data processing

def start_training_date(start_forecast, training_months):
# calculate start of training date from forecasting date and number of months required
...
# calculate end of training date from forecasting date
...
# calculate the start of the financial year
...
def forecast_months_count(start_date, end_date):
...
# === Notebook functions === Used to provide information within the notebook
...
# === Price calculation functions === Used to provide turn imported prices into single file suitable for analysis
...
# create the higest price-per-unit for each each BNF code, to ensure no duplication due to pack sizes (e.g. paracetamol 500mg tablets 32 and 100)
...
def process_dmd_data(dmd_raw_df, start_training, end_forecast):
# create monthly price list from dm+d data, to enable price calculations for each month of data in the forecast period
    
    # Generate the date range for the forecast period
    date_range = pd.date_range(start=start_training, end=end_forecast, freq='MS')
...
def merge_prices(dmd_price_df, dt_price_df):
# merge drug tariff and dm+d price lists
...
# === Data preparation functions === prepares the data for forecasting analysis

def create_forecast_bnf_codes(rx_df, full_price_df, start_training, end_training, top_x_percent, multiplier_limit):
# find top x by both items and actual cost, then removing those without a cost or outside of expected PPU multiplier limits (both x and 1/x).  Creates two dfs, one for individual forecasting, and one with excluded codes *WHAT TO DO*
    #UNIQUE ID IS BNF CODE

    #create distinct list of BNF codes
    full_price_bnf_codes = full_price_df[['bnf_code']].drop_duplicates()
    
    # Group by 'bnf_code' (unique_id) and aggregate sum of 'items' and 'actual_cost'
    grouped_rx_df = rx_df.groupby('unique_id').agg({'items': 'sum', 'actual_cost': 'sum'}).reset_index()
    ...
    top_items_bnf_codes = grouped_rx_df[grouped_rx_df['cumulative_items'] <= threshold_items]['unique_id']
    ...
    top_cost_bnf_codes = grouped_rx_df[grouped_rx_df['cumulative_actual_cost'] <= threshold_actual_cost]['unique_id']
    ...
    current_fy = financial_year_start(start_training) # Calculate start of financial year
    current_rx_ppu_df = (rx_df[(rx_df['ds'] >= current_fy) & (rx_df['ds'] <= end_training)] # Filter for current financial year and aggregate for average price per unit (PPU)
                         .groupby(['unique_id'], as_index=False)
                         .agg({'y': 'sum', 'actual_cost': 'sum'}))

    current_rx_ppu_df['avg_ppu'] = (current_rx_ppu_df['actual_cost'] / current_rx_ppu_df['y']) * 100  # Compute average price per unit (PPU) in pence
    ...
    merged_ppu_df = pd.merge(current_rx_ppu_df[['unique_id', 'avg_ppu']], median_price_df, left_on='unique_id', right_on='bnf_code') #Merge the calculated PPU with the median price per unit
    ...
    multiplier_bnf_codes = merged_ppu_df.loc[bnf_filter, 'unique_id'].tolist() #create list of BNF codes which are outside limits
    ...
    topx_rx_df = rx_df[rx_df['unique_id'].isin(top_bnf_codes)]
    non_topx_rx_df = rx_df[~rx_df['unique_id'].isin(top_bnf_codes)]
    ...
    # Return the filtered DataFrames, including the top_bnf_codes
    return topx_rx_df, non_topx_rx_df, top_bnf_codes


def fill_missing_combinations(final_df, min_month, max_month):
    # where there is significant prescribing variation across months, where some months may be zero (e.g. flu vaccines), this includes a row with a zero value for that preparation
...
def forecast_prescribing_data_ces(df, topx_rx_df, start_training, end_training, horizon, season_length):
# Function to use AutoARIMA to calculate forecasting
    ...
    # Set AutoARIMA as the model and environment variables
    os.environ['NIXTLA_ID_AS_COL'] = '1'  # Unique ID as column, not index
    models = [AutoCES(season_length=season_length)]  # AutoARIMA with specified season length
...
# create price list for rest of financial year, taking price from latest month
...
# filter BNF codes based on top x% of items AND actual cost
